...
function doRectanglesOverlap(rect1, rect2) {
  ...
  return !((right1 < rect2.left) || (rect1.left > right2)
  || (bottom1 < rect2.top) || (rect1.top > bottom2));
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  const openParenthesis = (isStartIncluded) ? '[' : '(';
  const closeParenthesis = (isEndIncluded) ? ']' : ')';
  return `${openParenthesis}${Math.min(a, b)}, ${Math.max(a, b)}${closeParenthesis}`;
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (Object.keys(bracketsPairs).includes(str[i])) {
      ...
      if (stack.length === 0 || bracketsPairs[stack.pop()] !== str[i]) {
        return false;
      }
    }
  }
...
function evaluateTicTacToePosition(position) {
  ...
  for (let j = 0; j < 3; j += 1) {
    if ((position[0][j]) && (position[0][j] === position[1][j])
  ...
  if (((position[0][0]) && (position[0][0] === position[1][1])
    && (position[1][1] === position[2][2]))
      || ((position[0][2]) && (position[0][2] === position[1][1])
